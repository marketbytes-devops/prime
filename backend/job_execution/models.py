...
class WorkOrder(models.Model):
    ...
    onsite_or_lab = models.CharField(
        max_length=20,
        choices=[("Onsite", "Onsite"), ("Lab", "Lab")],
        null=True,
        blank=True,
    )
...
class DeliveryNote(models.Model):
    work_order = models.ForeignKey(
        WorkOrder, on_delete=models.CASCADE, related_name="delivery_notes"
    )
    dn_number = models.CharField(max_length=50, unique=True, blank=True, null=True)
    signed_delivery_note = models.FileField(
    ...
    series = models.ForeignKey(
        NumberSeries, on_delete=models.SET_NULL, null=True, blank=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class DeliveryNoteItemComponent(models.Model):
    delivery_note_item = models.ForeignKey(
        DeliveryNoteItem, on_delete=models.CASCADE, related_name="components"
    )
...
class Invoice(models.Model):
    ...
    signed_invoice_file = models.FileField(
        upload_to="invoices-signed/",
        null=True,
        blank=True,
        help_text="Signed invoice file for raised status",
    )
