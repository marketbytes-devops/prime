        ...
        if recipient_list:
            for email, name in recipient_list:
                ...
                try:
                    ...
                    email_sent = True
                    logger.info(f"Invoice status change email sent to {email} for WO #{work_order.wo_number}")
                except Exception as e:
                    logger.error(f"Failed to send invoice status change email to {email} for WO #{work_order.wo_number}: {str(e)}")
        return email_sent
