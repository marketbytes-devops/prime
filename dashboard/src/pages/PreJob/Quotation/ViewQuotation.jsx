...
const handlePrint = async (quotation) => {
  // HARDCODED DEFAULT TERMS (fallback of last resort)
  const DEFAULT_TERMS = `
  ...
  // 1. Try custom terms from quotation
  if (quotation.terms?.content) {
    termsContent = quotation.terms.content;
  } 
  // 2. Try global template from API
  else {
    try {
      ...
      // Silently ignore - no global template exists
      console.warn("No global terms template found");
    }
  }

  // 3. Final fallback: use hard-coded DEFAULT_TERMS
