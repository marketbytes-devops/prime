...
const UpdateTermsAndConditions = () => {
  ...
  const fetchQuotationWithTerms = async () => {
    try {
      ...
      if (!quotationId) {
        ...
        setContent(res.data.content || DEFAULT_TERMS);
      ...
      setContent(DEFAULT_TERMS); // Fallback to default on error
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (quotationId) {
        // Save custom terms for specific quotation
        ...
        if (termsId) {
          ...
          setTermsId(res.data.id); // Update local state for future edits
        }
      }
